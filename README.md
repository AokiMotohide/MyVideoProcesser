...
### 3. Normal Map Smoothing（AI生成ノーマルマップのチラつき）

AIが生成したノーマルマップ（法線マップ）動画特有の、フレームごとの「法線ベクトル（表面の向き）のガタつき」を平滑化する専用モードです。

* **仕組み**: 画像の色（RGB）を単なる色ではなくXYZ空間における「向き（ベクトル）」として捉え直します。過去数フレームとのベクトルの平均を計算し、さらにベクトルの長さを必ず「1（単位長さ）」へと再正規化（Re-normalize）する処理を行うことで、ディテール（凹凸感）が潰れるのを防ぎつつノイズだけを強力に滑らかにします。
...
3. **Flicker Removal Type（除去モードの選択）**:
   ...
   * ノーマルマップ（法線マップ）のガタつきなら **「Normal Map Smoothing」** を選択します。
4. **Presets（強度の設定）**:
